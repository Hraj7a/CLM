    ...
    <div className="app">
      ...
      <nav className="navbar">
        <div className="navbar-inner">
          ...
          <div className={`navbar-links ${menuOpen ? "open" : ""}`}>
            <a href="#upload" className="nav-link" onClick={() => setMenuOpen(false)}>Analyze</a>
            {analysis && (
              <a href="#results" className="nav-link" onClick={() => setMenuOpen(false)}>Results</a>
            )}
            {analysis && (
              <a href="#qa" className="nav-link" onClick={() => setMenuOpen(false)}>Ask AI</a>
            )}
      ...
      <header className="hero" id="top">
        <div className="hero-inner">
          <p className="hero-eyebrow">AI-Powered · Contract Lifecycle Management</p>
          <h1 className="hero-title">Contract Review Dashboard</h1>
          <p className="hero-desc">
            Upload a contract to extract key terms, identify risks, and ask questions about the document using AI.
          </p>
        </div>
      </header>
      ...
      <main className="container">
        ...
        <section className="card upload-section" id="upload">
          <div className="section-header">
            ...
            <p className="section-hint">DOCX format is supported for this version</p>
          </div>
        ...
        {analysis && (
          <div id="results">
            ...
            <section className="metrics-section">
              <div className="metrics-grid">
                <div className="metric-card">
                  ...
                  <div className={`metric-value risk-badge ${summary?.overall_risk_level?.toLowerCase()}`}>
                    {summary?.overall_risk_level || "Unknown"}
                  </div>
                </div>
                <div className="metric-card">
                  <span className="metric-label">Risk Score</span>
                  <div className="metric-value">{summary?.risk_score ?? "N/A"}</div>
                </div>
                <div className="metric-card">
                  <span className="metric-label">Parties</span>
                  <div className="metric-value">{summary?.parties_count ?? 0}</div>
                </div>
                <div className="metric-card">
                  <span className="metric-label">Missing Clauses</span>
                  <div className="metric-value">{summary?.missing_clauses_count ?? 0}</div>
                </div>
              </div>
            </section>
            ...
            <section className="card overview-card">
              <h2 className="card-title">{summary?.contract_title || analysis.filename}</h2>
            ...
            <section className="grid-section">
              <div className="card info-card">
                ...
                <div className="info-list">
                  <Info label="Effective Date" value={summary?.effective_date} />
                  <Info label="Expiration Date" value={summary?.expiration_date} />
              ...
              <div className="card info-card">
                ...
                <div className="info-list">
                  <Info label="Summary" value={extraction?.payment_terms?.summary} />
                  <Info label="Payment Frequency" value={extraction?.payment_terms?.payment_frequency} />
                  <Info label="Late Payment Penalty" value={extraction?.payment_terms?.late_payment_penalty} />
                </div>
              </div>
            </section>
                  ...
                  <tbody>
                    {(risk?.missing_clauses || []).map((clause, i) => (
                      <tr key={i}>
                        ...
                        <td>
                          <span className={`risk-level ${clause.risk_level?.toLowerCase()}`}>
                            {clause.risk_level}
                          </span>
                        </td>
            ...
            <section className="card qa-section" id="qa">
              ...
              <p className="section-hint">Ask anything about the contract in plain language.</p>
              <div className="qa-wrapper">
                <input
                  ...
                  className="qa-input"
                />
                <button onClick={askQuestion} disabled={asking} className="btn btn-primary">
                  {asking ? (
